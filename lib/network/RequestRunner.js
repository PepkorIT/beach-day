...
var RequestRunner = (function () {
    ...
    RequestRunner.run = function (call, env) {
        ...
        if (!requestPassed) {
            ...
            var currSpecId_1 = BeachDayReporter_1.ReporterAPI.getCurrentSpecId();
            RequestRunner.request(options_1, function (error, sourceRes, body) {
                ...
                promise.then(function (response) {
                    if (response) {
                        ...
                        TestUtils_1.TestUtils.throwImplementationError("requestCallback did not return a response");
                    }
                    try {
                        // Ensure the same tests is still running
                        if (currSpecId_1 != BeachDayReporter_1.ReporterAPI.getCurrentSpecId()) {
                    ...
                    TestUtils_1.TestUtils.throwImplementationError(message);
                    env.done();
                });
            });
        }
    ...
    RequestRunner.runPoll = function (call, env, pollComplete, maximumRunTime) {
        ...
        var maxCallTime = BeachDayReporter_1.ReporterAPI.getReporterConfig().maxTestTime;
        ...
        env.done = function () {
            ...
            if (response.complete) {
                ...
                if (currentLength + response.nextPollDelay >= maximumRunTime) {
                    TestUtils_1.TestUtils.throwExpectError("Poll maximum run time met");
                    env.done = originalDone;
                    env.done();
                }
                else {
                    // Set the max test time to what has passed + the nextPollDelay + the max call time
                    BeachDayReporter_1.ReporterAPI.overrideSpecMaxTestTime(currentLength + response.nextPollDelay + maxCallTime);
